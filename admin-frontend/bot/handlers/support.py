...
async def cmd_support(message: Message):
    ...
    if language == "ru":
        await message.answer(
            ...
            "<i>Напишите сообщение или отправьте скриншот:</i>",
        ...
        await message.answer(
            ...
            "<i>Write a message or send a screenshot:</i>",
            reply_markup=get_cancel_keyboard(language)
        )
...
async def save_support_message(
    ...
    Args:
        ...
        message_text: Message text (or caption for photos)
        is_from_user: True if message is from user, False if from admin
        admin_id: Admin ID if message is from admin
        photo_file_id: Telegram file_id if message contains a photo
    ...
    async with async_session_maker() as session:
        ...
        # Include photo info in message if present
        full_message = message_text
        if photo_file_id:
            full_message = f"[PHOTO:{photo_file_id}]\n{message_text}" if message_text else f"[PHOTO:{photo_file_id}]"
...
async def handle_support_message(message: Message, user_id: int, photo_file_id: str = None):
    """
    ...
    Args:
        ...
        photo_file_id: Optional photo file_id if message contains a photo
    """
    language = await user_service.get_user_language(user_id)
    
    try:
        # Get message text (could be caption for photo messages)
