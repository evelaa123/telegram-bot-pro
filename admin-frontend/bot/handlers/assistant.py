...
async def process_alarm_time(message: Message, state: FSMContext):
    ...
    # Get user's timezone (default Moscow)
    user_tz_name = db_user.settings.get("timezone", "Europe/Moscow") if db_user.settings else "Europe/Moscow"
    try:
        user_tz = ZoneInfo(user_tz_name)
    except Exception:
        user_tz = ZoneInfo("Europe/Moscow")
    
    # Calculate next alarm time in user's timezone, then convert to UTC
    now_user_tz = datetime.now(user_tz)
    alarm_time_local = now_user_tz.replace(hour=hours, minute=minutes, second=0, microsecond=0)
    
    # If time already passed today, schedule for tomorrow
    if alarm_time_local <= now_user_tz:
        alarm_time_local += timedelta(days=1)
    
    # Convert to UTC for storage
    alarm_time = alarm_time_local.astimezone(timezone.utc)
